...
common_labs <- labs(fill = "Sujet moins préféré")

# Each plot with consistent theming and labels
p1 <- clean_data |>
  ggplot(aes(x = "", fill = unfav_subject)) +
...
combined_plot <- (p1 | p2) / (p3 | p4) +
  ...
  plot_annotation(title = "Sujet moins préféré par type MBTI") &
  theme(legend.position = "bottom")

# Save
ggsave("unfav_subject_graph.png", 
       plot = combined_plot, 
       path = "cache", 
       width = 6, 
       height = 5)
