...
common_labs <- labs(fill = "Programme en lycée")

# Each plot with consistent theming and labels
p1 <- clean_data |>
  ggplot(aes(x = "", fill = school_program)) +
...
combined_plot <- (p1 | p2) / (p3 | p4) +
  ...
  plot_annotation(title = "Programme en lycée par type MBTI") &
  theme(legend.position = "bottom")

# Save
ggsave("school_program_graph.png", 
       plot = combined_plot, 
       path = "cache", 
       width = 6, 
       height = 5)
